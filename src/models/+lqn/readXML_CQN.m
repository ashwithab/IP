function [myCQN, entryObj, entryGraphs, processors, providers, providerClasses] = readXML_CQN(filename,verbose)
% Q = READXML_CQN(A) reads an XML file A, that holds the  
...
for i = 1:size(processors,1)
    ...
    for j = 1:size(processors(i).tasks(1).actNames ,1)
        ...
        thisActCall.probs = [];
        thisEntryObj.actCalls = [thisEntryObj.actCalls; thisActCall];
    end
    entryObj = [entryObj; thisEntryObj]; 
    
    % initialize entryObj graphs
    thisEntryObjgraph = entryGraph(processors(i).name, M, K, processors(i).tasks(1).initActID);
    entryGraphs = [entryGraphs; thisEntryObjgraph]; 
end
...
for k = 1:K
    ...
    [fullP, fullMeanDemands, myCurProcID, visited, currentClass, classMatch, entryObj, entryGraphs, providerClasses] = ...
                        ...
                        LQNprocIdx, LQNactIdx, entryObj, entryGraphs, providerNames, providerClasses);%
