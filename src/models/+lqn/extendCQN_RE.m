function [myCQNCSRE] = extendCQN_RE(myCQNCS, REs, verbose)
% Q = EXTEND_CQN_RE(A,B) extends a CQN model A to include random
% environments (RE). The extended model Q is returned as a CQNRE object.
%
% Parameters:
    ...
    for i = 1:nE
        for j = 1:size( REs(i).parameters, 1)
            ...
            if procIdx ~= -1 %&& actProcs{procIdx,2} ~= delayProcIndex%considers hardware processors only (not delay either)
                switch REs(i).parameters{j,2} %evaluate parameter to modify in this processor
                    case 'speed-factor'
                        for k = 1:REs(i).numStages
                            %relevStages = find(stageIndex(:,i)==k)';
                            for l = find(stageIndex(:,i)==k)'
                                envRates{l}(procIdx,:) = envRates{l}(procIdx,:)*REs(i).parameters{j,3}(k);
                            end
                        end
                    case 'multiplicity'
                        for k = 1:REs(i).numStages
                ...
                warning(sprintf('Processor % not a hardware processor', elemID));
            end
        end
    end


    myCQNCSRE = CQNRE(M, K, E, N, ENV, envS, ...
                            envRates, sched, envP, NK, myCQNCS.classMatch, refNodes, fullResetRules, nodeNames, classNames);
else
   error(['Error: XML file ',filenameRE,' empty or unexistent']);
   myCQNCSRE = [];
end
