...
% PARSEXML_LQN(A) parses an XML file A containing a PMIF model
%
% Parameters: 
...
% 
% Output: 
% qn:           a QNM object that represents the PMIF model
%
...
for i = 0:serverList.getLength()-1
    ...
    if serverNode.getNodeType() == Node.ELEMENT_NODE
        ...
        tempServer = server(name, quantity, scheduling);
...
for i = 0:workUnitServerList.getLength()-1
    ...
    if serverNode.getNodeType() == Node.ELEMENT_NODE
        ...
        tempServer = workUnitServer(name, quantity, scheduling,serviceTime,timeUnits);
...
for i = 0:closedWorkloadList.getLength()-1
    ...
    if workloadNode.getNodeType() == Node.ELEMENT_NODE
        ...
        tempWorkload = closedWorkload(name, numberJobs, thinkTime, thinkDevice, timeUnits);
...
for i = 0:demandServiceRequestList.getLength()-1
    ...
    if demandNode.getNodeType() == Node.ELEMENT_NODE
        ...
        timeUnits = char(workloadElement.getAttribute('TimeUnits')); 
        tempDemand = demandServiceRequest(workloadName, serverID, serviceDemand, numberVisits, timeUnits);
...
for i = 0:workUnitServiceRequestList.getLength()-1
    ...
    if demandNode.getNodeType() == Node.ELEMENT_NODE
        ...
        serverID = char(demandElement.getAttribute('ServerID')); 
        numberVisits = str2num(char(demandElement.getAttribute('NumberOfVisits')));
        tempDemand = workUnitServiceRequest(workloadName, serverID, numberVisits);
        tempDemand = addTransits(tempDemand, demandNode); 
...
qn = QNM(servers, workUnitServers, closedWorkloads, ...
