classdef CQNREPHCoxSRP
% CQNRECOX defines an object that represents a Closed Multi-Class Queueing
% Network with Class Switching, Coxian service times, and simultaneous
% resource posession in a Random Environment (RE), 
...
properties
   M;           % number of stations (int)
   Mp;          % number of passive resource (int)
   K;           % number of classes (int)
   E;           % number of environmental stages (int)
   ...
   Sp;          % number of servers in passive stations in each stage (Ex1 cell with Mpx1 int entries)
   W;           % requirement of passive resources (dim 3) for each execution 
                % of each job class (dim 2) in each active resource (dim 1). 
                % (MxKxMp int)
   mu;          % transition rates for each of the Coxian service phases 
                % for each job class in each station and stage
                % (ExMxK cell with n_{e,i,k}x1 double entries)
...
%constructor
function obj = CQNREPHCoxSRP(M, Mp, K, E, N, ENV, S, Sp, W, mu, phi, sched, P, NK, classMatch, refNodes, resetRules, nodeNames, classNames)

    if nargin < 11
        disp('Too few arguments for a CQN_RE_Cox');
        obj = [];
    else
        obj.M = M;
        obj.Mp = Mp;
