...
% Network with Class Switching, Coxian service times, and simultaneous resource possession. 
...
properties
   ...
   S;           % number of servers per station (Mx1 int)
   Sp;          % number of servers in passive stations (Mpx1 int)
   W;           % requirement of passive resources (dim 2) for each execution 
...
%constructor
function obj = CQNCoxSRP(M, Mp, K, N, S, Sp, W, mu, phi, sched, P, NK, classMatch, refNodes, nodeNames, classNames)
    if(nargin > 0)
        obj.M = M;
        obj.Mp = Mp;
        obj.K = K;
