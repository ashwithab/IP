function [QN, ymean_emb, piENVsum, iters,runtime] = CQN_REPH_SRP_fluid_ps(myCQN, max_iter, delta_max, yinit, verbose)
% Q = CMCQN_CS_REPH_FLUID_PS(myCQN) computes the fixed point Q of the ODE 
...
% (CMCQNCSREPHCox) myCQN. 
% 
% Parameters: 
% myCQN:        CMCQNCSREPHCox model to analyze
...
Mp = myCQN.Mp;      % number of passive stations
    ...
    for e=[1:h-1 h+1:E] %sink
        if envP(h,e) > 0 
            ...
            if strcmp(resetRules{h,e}, 'fullReset')
                R{h,e} = zeros(sum(sum(phases(:,:,h)))+Mp, sum(sum(phases(:,:,e)))+Mp); % size = number of queues + total number of service phases in that stage 
                ...
                R{h,e} = eye(sum(sum(phases(:,:,h)))+Mp, sum(sum(phases(:,:,e)))+Mp );
            end
        end
    end
end
...
for e = 1:E
    if nargin < 10 || isempty('yinit')
        y0 = [];
        z0 = zeros(Mp,1); %passive resources
        ...
        y0 = [y0; z0]; % add passive resources to state description (Mp*K states)
...
for e =1:E
    %[ode_h{e}, ~] = CQN_RE_SRP_fluid_analysis_ps(N, reshape({Lambda{e,:,:}},M,K), reshape({Pi{e,:,:}},M,K), P{e}, S{e});
    [ode_h{e}, ~] = CQN_SRP_fluid_analysis_ps(N, reshape({Lambda{e,:,:}},M,K), reshape({Pi{e,:,:}},M,K), P{e}, S{e}, Sp{e}, W);
end
