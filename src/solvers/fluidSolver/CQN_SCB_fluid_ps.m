function [QN, ymean,iters,runtime] = CQN_SCB_fluid_ps(myCQN, max_iter, delta_max, yinit, verbose)
...
if nargin < 10 || isempty('yinit')
    y0 = [];
    z0 = zeros(nx,1); % state of blocking jobs in station where blocking occurs
    ...
    for i = 1:M;
        for k = 1:K
            if match(i,k) > 0
                ...
                for w = find(V(i,:,k)>0)
                    z0(q_indices(w,k)) = z0(q_indices(w,k)) + toAssign; 
                end
...
while goon > 0 && solveiters <= max_iter
    ...
    % init ode
    [ode_h, ~] = CQN_SCB_fluid_analysis_ps(N, Lambda, Pi, P, S, V);
