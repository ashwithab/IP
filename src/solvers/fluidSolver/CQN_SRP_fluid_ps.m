function [QN, ymean,iters,runtime] = CQN_SRP_fluid_ps(myCQN, max_iter, delta_max, yinit, verbose)
...
P = myCQN.P;
S = myCQN.S;
Sp = myCQN.Sp;  % multiplicity of passive resources
W = myCQN.W;    % passive resource requirements
...
pasInd = zeros(M, K); % indicator of active resource/class that use passive resources
for i = 1:M
    for k = 1:K
...
if nargin < 10 || isempty('yinit')
    y0 = [];
    %z0 = zeros(Mp*K,1); %passive resources
    ...
    for i = 1:M;
        for k = 1:K
            if match(i,k) > 0
                ...
                for w = find(W(i,:,k)>0)
                    %z0((w-1)*K + k) = z0((w-1)*K + k) + toAssign; 
                    z0(w) = z0(w) + toAssign; 
                end
                assigned(k) = assigned(k) + toAssign;
                
            else
                y0 = [y0; zeros(phases(i,k),1)];
            end
        end
...
while goon > 0 && solveiters <= max_iter
    ...
    % init ode
    [ode_h, ~] = CQN_SRP_fluid_analysis_ps(N, Lambda, Pi, P, S, Sp, W);
