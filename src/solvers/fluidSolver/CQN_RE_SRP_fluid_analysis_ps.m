function [ode_h,q_indices] = CQN_RE_SRP_fluid_analysis_ps(N, Mu, Phi, P, S)
% CMCQN_CS_RE_FLUID_ANALYSIS_PS provides the methods to perform the ODE 
% analysis of a Closed Multi-Class Queueing Network with Class Switching
% with Coxian Service Times in a Random Environment (CMCQNCSRECox). 
                ...
                for l = 1:K
                    ...
                    for k = 1:Kic(i,c)
                        ...
                        if x(xic+k-1) > 0 && n(i) > S(i)
                            rates = [rates; Phi{i,c}(k) * P((i-1)*K+c,(j-1)*K+l) * Mu{i,c}(k) * x(xic+k-1)/n(i) * S(i);]; 
                        elseif x(xic+k-1) > 0
                            rates = [rates; Phi{i,c}(k) * P((i-1)*K+c,(j-1)*K+l) * Mu{i,c}(k) * x(xic+k-1);];
                        else
                            rates = [rates; 0;]; 
                        end
                    end
