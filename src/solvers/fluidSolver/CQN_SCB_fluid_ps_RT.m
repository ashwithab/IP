function RT = CQN_SCB_fluid_ps_RT(myCQN, max_iter, delta_max, y0, RTrange, verbose)
...
%     end
    ...
    [newOde_h, ~] = CQN_SCB_fluid_analysis_ps(N, reshape({newLambda{:,:}},M,newK), reshape({newPi{:,:}},M,newK), newP, S, newV);
    ...
    for i = 1:length(outputStat)
        ...
        for w = find(V(outputStat(i),:,k)>0) % resources blocked by class-k jobs in output station i
            idxKp1 = sum(sum(newPhases(1:w-1,:))) + sum(newPhases(w,1:idxArtCl(outputClass(i))-1 ));            % base index of artifial class outputClass(i), stat w
            idxk = sum(sum(newPhases(1:w-1,:))) + sum(newPhases(w,1:outputClass(i)-1)); % base index of original class outputClass(i), stat w
            addY( idxKp1 + 1 ) = newFluid(i); % add blocking fluid to artificial class outputStat(i)
            addY( idxk+1:idxk+newPhases(w, outputClass(i)) ) = -newFluid(i); % remove blocking fluid from outputClass(i), all phases (should be only one)
        end
    end
    ...
    %indices new classes in all stations but delay -- consider non blocking
    %stations only for this class
    idxN = []; 
    for i = 1:M
        if i ~= delayNode && sum(V(:,i,k)) == 0 
    ...
    if iter > max_iter
        fprintf('Maximum number of iterations reached when computing the response time distribution for job chain %d.\n Response time distributions may be affected numerically.\n',k);
    end
    if verbose > 1
