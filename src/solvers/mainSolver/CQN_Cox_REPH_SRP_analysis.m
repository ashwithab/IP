function [Qavg, U, Ravg, Xavg, resEntry, RT_CDF,resEntry_CDF] = CQN_Cox_REPH_SRP_analysis(myCQN, entry, entryGraphs, processors, max_iter, delta_max, RT, RTrange, verbose)
...
y0 = [];
[Q, ymean_emb, piemb, ~, ~] = CQN_REPH_SRP_fluid_ps(myCQN, max_iter, delta_max, y0, verbose);

%% RT CDF analysis 
if RT >= 1
    RT_CDF = CQN_RE_SRP_fluid_ps_RT(myCQN, max_iter, delta_max, ymean_emb, piemb, RTrange, verbose);
    if RT == 2
    ...
    for i = 1:myCQN.M
        ...
        if delayNodes(i) == 1
            ...
            for k = 1:myCQN.K
                ...
                Xservice{e,i,k} = zeros(phases(e,i,k),1);
                for f = 1:phases(e,i,k)
                    Xfull(i,k) = Xfull(i,k) + Qfull(idx+f)*Lambda{e,i,k}(f)*Pi{e,i,k}(f)/xi*min(xi,myCQN.S{e}(i));
                    Xservice{e,i,k}(f) = Qfull(idx+f)*Lambda{e,i,k}(f)/xi*min(xi,myCQN.S{e}(i));
                end 
            end
        end
    end
