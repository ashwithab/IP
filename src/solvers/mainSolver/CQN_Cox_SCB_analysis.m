function [Q, U, R, X,resEntry, RT_CDF,resEntry_CDF] = CQN_Cox_SCB_analysis(myCQN, entry, entryGraphs, processors, max_iter, delta_max, RT, RTrange, verbose)
% [Q, U, R, X] = CQN_COX_SCB_ANALYSIS(A) computes the stationary performance
...
y0 = [];
[Q, ymean_emb,~,~] = CQN_SCB_fluid_ps(myCQN, max_iter, delta_max, y0, verbose);
if RT >= 1
    RT_CDF = CQN_SCB_fluid_ps_RT(myCQN, max_iter, delta_max, ymean_emb{end}, RTrange, verbose);
    if RT == 2
        warning('Response time distributions at entry level not supported for SRP models.');
    end
    RT_CDF_entry = [];
else
    RT_CDF = [];
...
for i = 1:myCQN.M
    if delayNodes(i) == 1
        ...
        xi = sum(Q(i,B(i,:)==0)); %number of non-blocking jobs in the station
        Si = max(0, myCQN.S(i)-sum(Q(i,B(i,:)==1)) ); % remove capacity stolen by blocking jobs 
        if xi>0
            for k = 1:myCQN.K
                if B(i,k) == 0 % non-blocking jobs only
                    ...
                    Xservice{i,k} = zeros(phases(i,k),1);
                    for f = 1:phases(i,k)
                        Xfull(i,k) = Xfull(i,k) + Qfull(idx+f)*Lambda{i,k}(f)*Pi{i,k}(f)/xi*min(xi, Si);
                        Xservice{i,k}(f) = Qfull(idx+f)*Lambda{i,k}(f)/xi*min(xi, Si);
                    end
                    if Xfull(i,k) < precisionX
                        Xfull(i,k) = 0;
                        Xservice{i,k} = zeros(phases(i,k),1); 
