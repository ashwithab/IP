function [Q, U, R, X,resEntry, RT_CDF,resEntry_CDF] = CQN_Cox_SRP_analysis(myCQN, entry, entryGraphs, processors, max_iter, delta_max, RT, RTrange, verbose)
% [Q, U, R, X] = CQN_COX_SRP_ANALYSIS(A) computes the stationary performance
...
y0 = [];
[Q, ymean_emb,~,~] = CQN_SRP_fluid_ps(myCQN, max_iter, delta_max, y0, verbose);
if RT >= 1
    RT_CDF = CQN_SRP_fluid_ps_RT(myCQN, max_iter, delta_max, ymean_emb{end}, RTrange, verbose);
    if RT == 2
